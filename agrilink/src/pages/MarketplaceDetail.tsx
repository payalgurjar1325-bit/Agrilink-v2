...
export default function MarketplaceDetail() {
  ...
  if (listing === undefined) return <LoadingState label="Loading listing..." />;
  if (listing === null) {
    return (
      <div className="mx-auto max-w-3xl px-4 py-16 text-center">
        <h2 className="text-xl font-semibold text-soil-900">Listing not found</h2>
        <Link to="/marketplace" className="mt-3 inline-block text-field-700 hover:underline">Back to Marketplace</Link>
      </div>
    );
  }
      ...
      <div className="overflow-hidden rounded-card border border-soil-100 bg-white">
        ...
        <div className="p-6">
          ...
          <div className="mt-5 grid grid-cols-2 gap-4 border-y border-soil-100 py-5 sm:grid-cols-4">
            ...
            <div>
              <div className="text-xs text-soil-900/50">Quantity</div>
              <div className="font-semibold text-soil-900">{listing.quantity} {listing.quantityUnit}</div>
            </div>
            <div>
              <div className="flex items-center gap-1 text-xs text-soil-900/50"><MapPin size={11}/> Location</div>
              <div className="font-semibold text-soil-900">{listing.location}, {listing.district}</div>
            </div>
            <div>
              <div className="flex items-center gap-1 text-xs text-soil-900/50"><Calendar size={11}/> Harvested</div>
              <div className="font-semibold text-soil-900">{formatDate(listing.harvestDate)}</div>
            </div>
          </div>

          <div className="mt-5">
            <h3 className="flex items-center gap-1.5 font-semibold text-soil-900"><Package size={16}/> Description</h3>
            <p className="mt-1.5 text-sm leading-relaxed text-soil-900/70">{listing.description}</p>
          </div>

          <div className="mt-6 flex flex-col gap-3 sm:flex-row">
            <Button fullWidth onClick={() => setShowEnquiry(true)}>Send Enquiry</Button>
            <Button fullWidth variant="secondary" onClick={() => setShowEnquiry(true)}>Contact Farmer</Button>
          </div>
        </div>
      </div>

      <Modal open={showEnquiry} onClose={() => { setShowEnquiry(false); setSent(false); }} title={sent ? 'Enquiry Sent' : 'Send Enquiry'}>
        {sent ? (
          ...
          <div className="space-y-4">
            ...
            <Button fullWidth onClick={() => setSent(true)}>Confirm & Send</Button>
          </div>
        )}
      </Modal>
    </div>
