...
export default function Register() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    login({ name: role === 'farmer' ? farmerForm.name || 'Farmer' : buyerForm.name || 'Buyer', role: role as 'farmer' | 'buyer' });
  ...
  return (
    <div className="mx-auto max-w-xl px-4 py-14 sm:px-6">
      <div className="mb-6 flex flex-col items-center">
        ...
        <h1 className="mt-3 text-2xl font-semibold text-soil-900">Create your account</h1>
        <p className="mt-1 text-sm text-soil-900/60">Join AgriLink as a farmer or a buyer.</p>
      </div>
      ...
      {!role ? (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <button onClick={() => setRole('farmer')} className="flex flex-col items-center gap-3 rounded-card border border-soil-100 bg-white p-7 hover:border-field-500">
            ...
            <span className="font-semibold text-soil-900">I am a Farmer</span>
          </button>
          <button onClick={() => setRole('buyer')} className="flex flex-col items-center gap-3 rounded-card border border-soil-100 bg-white p-7 hover:border-field-500">
            ...
            <span className="font-semibold text-soil-900">I am a Buyer</span>
          </button>
        </div>
      ) : (
        <form onSubmit={handleSubmit} className="space-y-4 rounded-card border border-soil-100 bg-white p-5">
          {role === 'farmer' ? (
            <>
              ...
              <div className="grid grid-cols-2 gap-4">
                <Select label="State" placeholder="Select" value={farmerForm.state} onChange={(e) => setFarmerForm((f) => ({ ...f, state: e.target.value }))} options={[...new Set(markets.map((m) => m.state))].map((s) => ({ value: s, label: s }))} />
                <Select label="District" placeholder="Select" value={farmerForm.district} onChange={(e) => setFarmerForm((f) => ({ ...f, district: e.target.value }))} options={[...new Set(markets.map((m) => m.district))].map((d) => ({ value: d, label: d }))} />
              </div>
          ...
          <div className="flex gap-3">
            <Button type="button" variant="secondary" onClick={() => setRole('')}>Back</Button>
      ...
      <p className="mt-5 text-center text-sm text-soil-900/60">
        Already have an account? <Link to="/login" className="font-medium text-field-700 hover:underline">Login</Link>
      </p>
    </div>
  );
}
